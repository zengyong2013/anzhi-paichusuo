...
<!--                            <img src="../../assets/main/logo-dark.png" alt="">-->
                             <div style="height:100px;line-height100px;width: 360px;text-align: center;font-size: 30px">海淀看守所</div>
                        </div>
                        <Form v-if="isLogin" ref="formValidate1" class="login-from" :model="loginJson" :rules="loginCheck">
                            <FormItem prop="userName">
                                <Input prefix="ios-person-outline" v-model="loginJson.userName" autofocus clearable placeholder="请输入用户名" />
                            </FormItem>
                        ...
                        <Form v-else ref="formValidate2" class="login-from" :model="registerJson" :rules="registerCheck">
                            <FormItem prop="mail">
                                <Input prefix="ios-mail-outline" v-model="registerJson.mail" autofocus clearable placeholder="请输入邮箱" />
                            </FormItem>
                            <FormItem prop="userName">
                                <Input prefix="ios-person-outline" v-model="registerJson.userName" clearable placeholder="请输入用户名" />
                            </FormItem>
                            <FormItem prop="password">
        ...
        methods: {
            ...
            login (name) {
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        this.loading = true
                        this.reqPost('/user/login' ,
                            {name : this.loginJson.userName , password : this.loginJson.password})
                            .then(res => {
                                console.log(res)
                                this.loading = false
                                if(res.code != 2000) return this.$Message.warning(res.msg)
                                this.$Message.success('欢迎'+res.data.userInfo.name);
...
<style lang="less">
    .login-page {
        ...
        .demo-2 .large-header {
            background-image: url('../../assets/login/login-bg-2.jpg');
